...
class App(Tk):
    ...
    def generate_csv(self):
        ...
        try:
            epoch_start_time = start_time_seconds - offset
            if epoch_start_time < 0:
                raise ValueError("The calculated timestamp is negative.")
            start_time = datetime.fromtimestamp(epoch_start_time)
            current_time_seconds = int((now - start_time).total_seconds())
